...
fstream wr;  //use to write cv and cd in file which name is line.dat
		...
		ostream &out)					// output stream
{
	...
	//cout<<"call ann_split::print"<<endl;
	child[ANN_LO]->print(level+1, out);	// print high child
	//out << "    ";
	//for (int i = 0; i < level; i++)		// print indentation
	//	out << "..";
	//out << "Split cd=" << cut_dim << " cv=" << cut_val;
	if((cut_dim==5)||(cut_dim==4))
	{
		wr<<cut_dim<<" "<<cut_val<<" "<<cut[0]<<" "<<cut[1]<<" "<<cut[2]<<" "<<cut[3]<<endl;
	}
	//out << " lbnd=" << cd_bnds[ANN_LO];
	//out << " hbnd=" << cd_bnds[ANN_HI];
	//out<<"  cut: "<<cut[0]<<" "<<cut[1]<<" "<<cut[2]<<" "<<cut[3];
	//out << "\n";
	//child[ANN_LO]->print(level+1,out);
	child[ANN_HI]->print(level+1, out);	// print low child
}
	...
	ANNkd_splitter		splitter)		// splitting routine
{
	if (n <= bsp) {						// n small, make a leaf node
		...
		double s=bnd_box.high;
		double x=bnd_box.low;
		double z=bnd_box.left;
		double y=bnd_box.right;
		if(cd==6)  //modify the Rectangle bound of the low-subnode
		{
			bnd_box.high=cv;
			bnd_box.low=x;
			bnd_box.left=z;
		...
		{
			//cout<<"cd value is wrong   cd: "<<cd<<endl;
			bnd_box.high=s;
			bnd_box.low=x;
			bnd_box.left=z;
			bnd_box.right=y;
		...
		if(cd==6) //modify the Rectangle bound of the high-subnode
		{
			bnd_box.high=s;
			bnd_box.low=cv;
			bnd_box.left=z;
		...
		ANNkd_split *ptr = new ANNkd_split(cd, cv, lv, hv, lo, hi,s,x,z,y);
	...
	ANNsplitRule		split)			// splitting method
{
	...
	//cout<<"ts              :"<<bnd_box.low<<" "<<bnd_box.high<<" "<<bnd_box.left<<" "<<bnd_box.right<<endl; 
